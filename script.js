...
const dbReq = indexedDB.open("Peegel_V33_Final", 1);
...
function renderHistory() {
    ...
    tx.objectStore("sessions").getAll().onsuccess = e => {
        ...
        document.getElementById('history-container').innerHTML = list.map(s => `
            <div class="glass rounded-[30px] p-5 space-y-4 border border-white/5 shadow-xl">
                ...
                <div class="p-4 bg-green-500/5 rounded-2xl space-y-3 border border-green-500/10">
                    <div class="flex justify-between items-center text-[9px] font-black text-green-400 uppercase tracking-widest">
                        ...
                        ${s.audioClean ? `<button onclick="dl('${s.audioClean}', 'Puhas_${s.id}')" class="text-green-400 border border-green-400/20 px-2 py-0.5 rounded">Download</button>` : `<button id="proc-btn-${s.id}" onclick="processSilence(${s.id})" class="bg-blue-600/40 text-blue-200 px-3 py-1 rounded-lg">Eemalda vaikus</button>`}
                    </div>
